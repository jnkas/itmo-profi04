    ...
    if ($min_date <= $max_date) {
        ...
        foreach ($content_dir as $file) {
            ...
            if ($min_date <= $curr_date && $curr_date <= $max_date) {
                $curr_file = file_get_contents("tplt/$file",FILE_USE_INCLUDE_PATH);
        ...
        $output_string = "<p>Начало периода не может быть позднее конца периода!</p>";
    }
} else {
